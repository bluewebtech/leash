...
if( ( $pos = strrpos( DIR , WEB ) ) !== false ) {
	$main = substr_replace( DIR , SYSTEM , $pos , strlen( WEB ) ) . DS . SYSTEM . SYSTEM_EXT;
}
// -- Make sure the main.php application file exists before continuing
try {

	if( !file_exists( $main ) ) {
		throw new Exception( 'Main application file (' . SYSTEM . SYSTEM_EXT . ') does not exist.' );
	}
